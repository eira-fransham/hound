...
pub fn read_until_data<R: io::Read>(mut reader: R) -> Result<(WavSpecEx, u32)> {
    ...
    loop {
        let header = WavReader::read_chunk_header(&mut reader)?;
        match header.kind {
            ChunkKind::Fmt => {
                let spec = WavReader::read_fmt_chunk(&mut reader, header.len)?;
                spec_opt = Some(spec);
            }
            ChunkKind::Fact => {
            ...
            ChunkKind::Unknown => {
                ...
                reader.skip_bytes(header.len as usize)?;
            }
        }
        // If no data chunk is ever encountered, the function will return
        // via one of the ? macros that return an Err on end of file.
    }
}
    ...
    R: io::Read,
{
    ...
    fn read_fmt_chunk(reader: &mut R, chunk_len: u32) -> Result<WavSpecEx> {
        ...
        let n_samples_per_sec = reader.read_le_u32()?;
        ...
        let fmt_kind = match format_tag {
            PCM => {
                WavReader::read_wave_format_pcm(reader, chunk_len, &spec)?;
                FmtKind::PcmWaveFormat
            }
            ADPCM => return Err(Error::Unsupported),
            IEEE_FLOAT => {
                WavReader::read_wave_format_ieee_float(reader, chunk_len, &mut spec)?;
                FmtKind::PcmWaveFormat
            }
            EXTENSIBLE => {
                WavReader::read_wave_format_extensible(reader, chunk_len, &mut spec)?;
    ...
    fn read_wave_format_pcm(mut reader: R, chunk_len: u32, spec: &WavSpec) -> Result<()> {
        ...
        if is_wave_format_ex {
            ...
            let _cb_size = reader.read_le_u16()?;
    ...
    fn read_wave_format_ieee_float(
        mut reader: R,
        chunk_len: u32,
        spec: &mut WavSpec,
    ) -> Result<()> {
    ...
    fn read_wave_format_extensible(
        ...
        // ```
        let valid_bits_per_sample = reader.read_le_u16()?;
        let _channel_mask = reader.read_le_u32()?; // Not used for now.
        let mut subformat = [0u8; 16];
        reader.read_into(&mut subformat)?;
    ...
    pub fn new(mut reader: R) -> Result<WavReader<R>> {
        ...
        let (spec_ex, data_len) = read_until_data(&mut reader)?;
